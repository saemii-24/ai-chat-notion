    ...
    for (const key of Object.keys(obj)) {
      ...
      if (typeof val === "string") {
        if (isHex(val.trim())) {
          ...
          if (lname.includes("background") || lname.includes("bg"))
            out[key] = "var(--code-bg)";
          else if (lname === "color" || lname.includes("foreground"))
            out[key] = "var(--code-fg)";
...
const ChatMessageItem: React.FC<ChatMessageItemProps> = ({ message }) => {
  ...
  return (
    <div
      className={`flex w-full mb-8 ${isUser ? "justify-end" : "justify-start"} animate-in fade-in slide-in-from-bottom-2 duration-300`}
    >
      <div
        className={`flex max-w-[92%] md:max-w-[80%] gap-3 ${isUser ? "flex-row-reverse" : "flex-row"}`}
      >
        {/* Avatar */}
        <div
          className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg transition-colors ${
            isUser
              ? "bg-indigo-600"
        ...
        >
          <div
            className={`px-5 py-3.5 rounded-2xl shadow-sm border transition-colors ${
              isUser
          ...
          >
            {message.parts.map((part, idx) => (
              <div key={idx} className="space-y-4">
                ...
                {part.text && (
                  <div
                    className={`prose prose-slate max-w-none break-words text-sm md:text-base leading-relaxed ${isUser ? "prose-invert" : "dark:prose-invert"}`}
                  >
                    <ReactMarkdown
                      ...
                      components={{
                        code({
                          ...
                          return !inline && match ? (
                            ...
                            <code
                              className={`${isUser ? "bg-indigo-700 text-white" : "bg-slate-100 dark:bg-slate-800 text-indigo-600 dark:text-indigo-300"} px-1.5 py-0.5 rounded font-mono text-sm`}
                              {...props}
                            >
                              {children}
                            </code>
