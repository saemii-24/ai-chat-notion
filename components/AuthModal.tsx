...
interface AuthModalProps {
  onLogin: (email: string, provider?: "google" | "email") => void;
}
...
const AuthModal: React.FC<AuthModalProps> = ({ onLogin }) => {
  ...
  const handleGoogleLogin = async () => {
    setIsLoggingIn(true);
    try {
      const user = await CloudDBService.loginWithGoogle();
      onLogin(user.email, "google");
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (email && password) {
      ...
      try {
        let user;
        if (isLogin) {
          user = await CloudDBService.login(email, password);
        } else {
          // trying to register (if already exists, register() will sign in)
          user = await CloudDBService.register(email, password);
        }
        ...
        <div className="p-8 md:p-12">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              Next.js & Firebase 기반의 스마트 학습 환경
            </p>
          </div>
          ...
          <div className="space-y-3 mb-8">
            <button
              ...
              className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-bold py-4 rounded-2xl flex items-center justify-center gap-3 transition-all hover:bg-slate-50 dark:hover:bg-slate-700/50 active:scale-[0.98] disabled:opacity-50"
            >
              <img
