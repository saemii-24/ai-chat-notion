...
export default function Home() {
  ...
  const onSendMessage = async (
    ...
    try {
      ...
      const handleNotionFunctionCall = async (call: any) => {
        const callName = call?.name || call?.function?.name;
        if (!callName) return;
        const args = parseFunctionArgs(call?.args ?? call?.arguments);
        const callKey = `${callName}:${JSON.stringify(args)}`;
        if (handledFunctionCalls.has(callKey)) return;
        handledFunctionCalls.add(callKey);

        if (!notionConfig.enabled) {
          showToast("노션 동기화가 비활성화되어 있습니다.");
          return;
        }

        if (!notionConfig.apiKey) {
          showToast("노션 통합 토큰을 먼저 입력해주세요.");
          return;
        }

        try {
          if (callName === "save_word_to_notion") {
            if (!notionConfig.wordDbId) {
              showToast("단어 DB ID를 먼저 입력해주세요.");
              return;
            }
            ...
            if (words.length > 1) {
              await NotionService.saveWords(
                notionConfig.apiKey,
                notionConfig.wordDbId,
                { words },
              );
              showToast("단어가 모두 노션에 저장되었습니다.", "success");
            } else {
              const [word] = words;
              await NotionService.saveWord(
                notionConfig.apiKey,
                notionConfig.wordDbId,
                {
                  word: word?.word ?? "",
              ...
              showToast("단어가 노션에 저장되었습니다.", "success");
            }
          }

          if (callName === "save_sentence_to_notion") {
            if (!notionConfig.sentenceDbId) {
              showToast("문장 DB ID를 먼저 입력해주세요.");
              return;
            }
            await NotionService.saveSentence(
              notionConfig.apiKey,
              notionConfig.sentenceDbId,
              {
            ...
            showToast("문장이 노션에 저장되었습니다.", "success");
          }
        } catch (err) {
          console.error("Notion Save Error:", err);
          showToast("노션 저장에 실패했습니다.");
        }
      };
      ...
      for await (const chunk of stream) {
        if (chunk.functionCalls && Array.isArray(chunk.functionCalls)) {
          for (const call of chunk.functionCalls) {
            await handleNotionFunctionCall(call);
          }
        }
        if (chunk.text) {
